...
public partial class DHRuntimeViewCompiler : IViewCompiler
#pragma warning restore CA1852 // Seal internal types
{
    ...
    private Task<CompiledViewDescriptor> OnCacheMiss(string normalizedPath)
    {
        ...
        if (item.SupportsCompilation)
        {
            ...
            if (item.Descriptor?.Item != null &&
                DHChecksumValidator.IsItemValid(_projectEngine.FileSystem, item.Descriptor.Item))
            {
    ...
    private ViewCompilerWorkItem CreatePrecompiledWorkItem(string normalizedPath, CompiledViewDescriptor precompiledView)
    {
        ...
        // based on checksums.
        if (precompiledView.Item == null || !DHChecksumValidator.IsRecompilationSupported(precompiledView.Item))
        {
    ...
    protected virtual CompiledViewDescriptor CompileAndEmit(string relativePath)
    {
        ...
        {
            throw DHCompilationFailedExceptionFactory.Create(
                codeDocument,
    ...
    private string GetNormalizedPath(string relativePath)
    {
        ...
        {
            normalizedPath = DHViewPath.NormalizePath(relativePath);

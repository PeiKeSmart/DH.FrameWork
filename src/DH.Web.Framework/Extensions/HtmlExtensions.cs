...
namespace DH.Web.Framework.Extensions
{
    ...
    public static class HtmlExtensions
    {
            ...
            where TLocalizedModelLocal : ILocalizedLocaleModel
        {
            ...
            {
                tabStrip.AppendLine($"<a class=\"nav-link active\" data-tab-name=\"{standardTabName}\" href=\"#{standardTabName}\" data-toggle=\"pill\" role=\"tab\" aria-selected=\"false\">{localizationService.GetResource("Admin.Common.Standard")}</a>");
            }
            else
            {
                tabStrip.AppendLine($"<a class=\"nav-link\" data-tab-name=\"{standardTabName}\" href=\"#{standardTabName}\" data-toggle=\"pill\" role=\"tab\" aria-selected=\"false\">{localizationService.GetResource("Admin.Common.Standard")}</a>");
            }
            ...
            {
                // 语言
                var language = Language.FindById(locale.LanguageId);
                if (language == null)
                    throw new Exception("无法加载语言");
            ...
            {
                // 语言
                var language = Language.FindById(helper.ViewData.Model.Locales[i].LanguageId);
                if (language == null)

        ...
        public static IEnumerable<string> ExpandGlobs(IAsset asset, IWebHostEnvironment env)
        {
            ...
            if (asset.SourceFiles.Any())
            {
                foreach (string sourceFile in asset.SourceFiles)
                {
                    ...
                    XTrace.WriteLine($"获取数据ExpandGlobs111111：{outSourceFile}");

                    if (asset.ExcludeFiles.Count == 0 && provider.GetFileInfo(outSourceFile).Exists)
                    {
                        XTrace.WriteLine($"获取数据ExpandGlobs22222：{sourceFile}");
                    ...
                    else
                    {
                        ...
                        XTrace.WriteLine($"获取数据ExpandGlobs333333：{asset.ExcludeFiles.JoinAsString(",")}");
                        ...
                        XTrace.WriteLine($"获取数据ExpandGlobs444444：{virtualFilePaths.JoinAsString(",")}");

                        IEnumerable<string> fileMatches = globbingResult.Files.Select(f => f.Path);
                        XTrace.WriteLine($"获取数据ExpandGlobs555555：{fileMatches.JoinAsString(",")}");

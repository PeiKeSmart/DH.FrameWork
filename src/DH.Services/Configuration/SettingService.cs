...
public partial class SettingService : ISettingService {
    ...
    {
        var list = _cache.Get<IDictionary<String, IList<Setting>>>(DHSettingsDefaults.SettingsAllAsDictionaryCacheKey.ToString());
        if (list == null || !list.Any())
        {
            var settings = GetAllSettings();

            list = new Dictionary<string, IList<Setting>>();
            foreach (var s in settings)
            {
                ...
                if (!list.ContainsKey(resourceName))
                    // 第一次设置
                    list.Add(resourceName, new List<Setting>
                    {
                        settingForCaching
                    });
                else
                    ...
                    list[resourceName].Add(settingForCaching);
            }

            _cache.Set(DHSettingsDefaults.SettingsAllAsDictionaryCacheKey.ToString(), list);
        }

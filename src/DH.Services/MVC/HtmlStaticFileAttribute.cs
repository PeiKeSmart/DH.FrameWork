...
/// <summary>
/// 动态页面转静态页面  生成Html静态文件  支持MVC和PageModel
/// </summary>
/// <remarks>
/// 参考https://github.com/toolgood/StaticPage
/// </remarks>
...
public class HtmlStaticFileAttribute : ActionFilterAttribute, IPageFilter {
    ...
    /// 开发模式
    /// </summary>
    public static bool IsDevelopmentMode = DHSetting.Current.IsHtmlStaticDevelopmentMode;
    ...
    {
        CurrPageExpireMinutes = DHSetting.Current.HtmlStaticExpireMinutes;
    }
    ...
    private ActionResult GetStaticFileResult(FilterContext context)
    {
        ...
        if (File.Exists(filePath))
        {
            ...
            var etag = fi.LastWriteTimeUtc.Ticks;

            if (etag >= DateTime.UtcNow.AddMinutes(-ExpireMinutes).Ticks)
            {
                if (context.HttpContext.Request.Headers["If-None-Match"] == etag)
                {
                    return new StatusCodeResult(304);
    ...
    private string GetOutputFilePath(FilterContext context)
    {
        ...
        if (language != null)
        {
            l = $".{language.UniqueSeoCode}";
        }

        return Path.Combine(dir, t) + m + l + ".html";
    }

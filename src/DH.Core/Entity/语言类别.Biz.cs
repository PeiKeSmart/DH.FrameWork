...
public partial class Language : DHEntityBase<Language>
{
    ...
    public override void Valid(Boolean isNew)
    {
        ...
        // 检查唯一索引
        // CheckExist(isNew, __.UniqueSeoCode);
        // CheckExist(isNew, __.LanguageCulture);
    }
    ...
    protected override void InitData()
    {
        ...
        if (XTrace.Debug) XTrace.WriteLine("开始初始化Language[语言类别]数据……");
        ...
        if (XTrace.Debug) XTrace.WriteLine("完成初始化Language[语言类别]数据！");
    }
    ...
    /// <summary>根据Url缩写/标识查找</summary>
    /// <param name="uniqueSeoCode">Url缩写/标识</param>
    /// <returns>实体对象</returns>
    public static Language FindByUniqueSeoCode(String uniqueSeoCode)
    {
        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.UniqueSeoCode == uniqueSeoCode);

        return Find(_.UniqueSeoCode == uniqueSeoCode);
    }

    /// <summary>根据本地化语言标识查找</summary>
    /// <param name="languageCulture">本地化语言标识</param>
    /// <returns>实体对象</returns>
    public static Language FindByLanguageCulture(String languageCulture)
    {
        // 实体缓存
        if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.LanguageCulture == languageCulture);

        return Find(_.LanguageCulture == languageCulture);
    }
    ...
    /// <returns>实体对象</returns>
    public static IList<Language> FindByIds(String ids)
    {
        if (ids.IsNullOrWhiteSpace()) return new List<Language>();
    ...
    /// <returns></returns>
    public static IList<Language> GetWithPage(PageParameter pages)
    {
        if (Meta.Session.Count < 1000)
        {
            pages.TotalCount = Meta.Session.Count;
            return FindAllWithCache().OrderBy(e => e.DisplayOrder).Skip((pages.PageIndex - 1) * pages.PageSize).Take(pages.PageSize).ToList();
        }

        pages.Desc = false;
        pages.Sort = "DisplayOrder";
    ...
    public static IList<Language> Search(String languageCulture, String uniqueSeoCode, String key, PageParameter page)
    {
        var exp = new WhereExpression();
        ...
        if (!key.IsNullOrEmpty()) exp &= _.Name.Contains(key) | _.DisplayName.Contains(key) | _.EnglishName.Contains(key) | _.Flag.Contains(key) | _.Domain.Contains(key) | _.Remark.Contains(key) | _.CreateUser.Contains(key) | _.CreateIP.Contains(key) | _.UpdateUser.Contains(key) | _.UpdateIP.Contains(key);

        return FindAll(exp, page);
    }

    // Select Count(Id) as Id,Category From Language Where CreateTime>'2020-01-24 00:00:00' Group By Category Order By Id Desc limit 20
    //static readonly FieldCache<Language> _CategoryCache = new FieldCache<Language>(_.Category)
    //{
    ...
    /// <returns></returns>
    public static IEnumerable<Language> Searchss(string key, Int32 status, PageParameter page)
    {
        var exp = new WhereExpression();
        if (!key.IsNullOrEmpty()) exp &= _.Name.Contains(key) | _.DisplayName.Contains(key) | _.EnglishName.Contains(key);

        if (status > -1)
            exp &= _.Status == status.ToBoolean();

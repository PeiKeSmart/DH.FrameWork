...
public partial class SiteInfo : ISiteInfo, IEntity<ISiteInfo>
{
    ...
    /// <param name="sslEnabled">是否启用SSL</param>
    /// <param name="start">更新时间开始</param>
    /// <param name="end">更新时间结束</param>
    /// <param name="key">关键字</param>
    /// <param name="page">分页参数信息。可携带统计和数据权限扩展查询等信息</param>
    /// <returns>实体列表</returns>
    public static IList<SiteInfo> Search(Boolean? sslEnabled, DateTime start, DateTime end, String key, PageParameter page)
    {
        var exp = new WhereExpression();

        if (sslEnabled != null) exp &= _.SslEnabled == sslEnabled;

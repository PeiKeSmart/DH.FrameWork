...
public partial class GenericAttribute : IGenericAttribute, IEntity<IGenericAttribute>
{
    ...
    public static IList<GenericAttribute> Search(Int32 entityId, String keyGroup, DateTime start, DateTime end, String key, PageParameter page)
    {
        var exp = new WhereExpression();

        if (entityId >= 0) exp &= _.EntityId == entityId;
        if (!keyGroup.IsNullOrEmpty()) exp &= _.KeyGroup == keyGroup;

...
namespace DH.Entity {
    public partial class UrlRecord : DHEntityBase<UrlRecord>
    {
        ...
        /// <summary>根据路由参数查找</summary>
        /// <param name="slug">路由参数</param>
        /// <returns>实体对象</returns>
        public static UrlRecord FindBySlug(String slug)
        {
            if (slug.IsNullOrWhiteSpace()) return null;

            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.Slug == slug);
        ...
        {
            // 实体缓存
            if (Meta.Session.Count < 1000) return Meta.Cache.Find(e => e.EntityId == entityId && e.EntityName == entityName && e.LanguageId == languageId && e.IsActive)?.Slug;
        ...
        /// <summary>
        /// 接受十六进制字符串并将其转换为Unicode字符
        /// </summary>
        /// <param name="hexString">十六进制的四位数数字（例如00E7）。</param>
        /// <returns>Unicode字符，如字符串。</returns>
        protected static String ToUnichar(String hexString)
        {
            var b = new Byte[2];

            // 以十六进制作为文本并创建Unicode字符数
        ...
        /// <param name="convertNonWesternChars">指示是否应转换非西方字符的值</param>
        /// <param name="allowUnicodeCharsInUrls">指示是否允许Unicode字符的值</param>
        /// <returns>Result</returns>
        public static String GetSeName(String name, Boolean convertNonWesternChars, Boolean allowUnicodeCharsInUrls)
        {

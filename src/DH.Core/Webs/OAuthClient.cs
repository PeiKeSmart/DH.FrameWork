...
public class OAuthClient {
    ...
    public static OAuthClient Create(String name)
    {
        ...
        // SSO支持注销
        if (name.EqualIgnoreCase("ChuangChu") && client.LogoutUrl.IsNullOrEmpty()) client.LogoutUrl = "logout?client_id={key}&redirect_uri={redirect}&state={state}";
    ...
    /// <param name="UtId">客户Id</param>
    public virtual String Authorize(String redirect, Int32 ReferrerId, Int32 UtId, String state = null, Uri baseUri = null)
    {
        ...
        if (ReferrerId > 0)
        {
            AuthUrl = AuthUrl.AppendReturn(ReferrerId.ToString(), "ReferrerId");
        }
        if (UtId > 0)
        {
            AuthUrl = AuthUrl.AppendReturn(UtId.ToString(), "UtId");
        }
    ...
    public virtual String GetAccessToken(String code)
    {
        ...
        if (dic != null)
        {
            ...
            if (dic.ContainsKey("refresh_expires")) RefreshExpire = DateTime.Now.AddSeconds(dic["refresh_expires"].Trim().ToInt());
    ...
    protected virtual void OnGetInfo(IDictionary<String, String> dic)
    {
        ...
        if (dic.TryGetValue("issales", out str)) IsSales = str.ToBoolean();

        if (dic.TryGetValue("referrerid", out str)) ReferrerId = str.ToInt();

        if (dic.TryGetValue("ex1", out str)) Ex1 = str.ToInt();

        if (dic.TryGetValue("kefuid", out str)) KeFuId = str.ToInt();

...
public class LocalizationSettings : Config<LocalizationSettings> {
    ...
    /// 默认管理区域语言标识符
    /// </summary>
    public Int32 DefaultAdminLanguageId { get; set; }

    /// <summary>
    /// 使用图像选择语言
    /// </summary>
    public Boolean UseImagesForLanguageSelection { get; set; }

    /// <summary>
    /// 一个值，指示是否启用具有多种语言的SEO友好URL
    /// </summary>
    public Boolean SeoFriendlyUrlsForLanguagesEnabled { get; set; } = true;

    /// <summary>
    /// 指示是否应按客户区域检测当前语言的值（浏览器设置）
    /// </summary>
    public Boolean AutomaticallyDetectLanguage { get; set; } = true;

    /// <summary>
    /// 一个值，指示是否在应用程序启动时加载所有LocaleStringResource记录
    /// </summary>
    public Boolean LoadAllLocaleRecordsOnStartup { get; set; }

    /// <summary>
    /// 一个值，指示是否在应用程序启动时加载所有LocalizedProperty记录
    /// </summary>
    public Boolean LoadAllLocalizedPropertiesOnStartup { get; set; }

    /// <summary>
    /// 一个值，指示是否在应用程序启动时加载所有搜索引擎友好名称（slug）
    /// </summary>
    public Boolean LoadAllUrlRecordsOnStartup { get; set; }

    /// <summary>
    /// 一个值，指示我们是否应该忽略管理区域的RTL语言属性。
    /// 这对于使用RTL语言的店主来说很有用，但对于管理区域来说更喜欢LTR
    /// </summary>
    public Boolean IgnoreRtlPropertyForAdminArea { get; set; }
}

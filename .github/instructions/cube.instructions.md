...
本指令面向沛柯智能（PeiKeSmart）Web 快速开发框架 Pek.NCube（魔方），帮助 Copilot 在 .NET 项目中正确使用 Pek.NCube 进行 Web 管理后台开发。
...
DH.NCore（基础库）
       ↓
   DH.NCode（数据中间件）→ xcode.instructions.md
       ↓
   Pek.NCube（Web 快速开发框架）← 本指令
```

兼容性说明：当前 Pek.NCube 相关源码与历史文档中，部分 API、命名空间或默认配置名称仍可能保留 `NewLife.*` 标识。编写说明和示例时，应以项目主体 `Pek.NCube` / `DH.NCore` / `DH.NCode` 为准，但代码示例必须遵循仓库中的真实源码签名。

### 1.2 职责边界

| 层级 | 职责 | 指令文件 |
|------|------|---------|
| **DH.NCode** | 数据建模、ORM、实体增删改查 | `xcode.instructions.md` |
| **Pek.NCube** | Web 管理后台、控制器、视图、权限、菜单 | 本指令 |
...
在 Model.xml 中配置 `CubeOutput` 后，执行 `xcode` 会自动生成 Pek.NCube 所需代码：
- 区域类（`*Area.cs`）
- 控制器（`*Controller.cs`）
